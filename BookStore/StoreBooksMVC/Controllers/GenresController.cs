...
namespace StoreBooksMVC.Controllers
{
    public class GenresController : Controller
    {
        ...
        // GET: Genres
        public async Task<ActionResult> Index(string sortedQuery, string currentFilter)
        {
            ViewBag.CurrentSort = sortedQuery;
            ViewBag.NameSortParm = String.IsNullOrEmpty(sortedQuery) ? "GenreName—desc" : "";

            var genre = from g in db.Genres select g;

            switch (sortedQuery)
            {
                default:
                    genre = genre.OrderBy(g => g.GenreName);
                    break;
            }
